...
\begin{itemize}
\item \textbf{output}(y precisión, si la tiene) \code{método(argumentos)}: Descripción del método. Si un método no tiene argumentos o no regresa un output, su descripción omitirá esa sección.
\item \textbf{int32} \code{timestamp\_count}: El tamaño del búfer de eventos.
\end{itemize}

Las propiedades modificables del TDC son:
\begin{itemize}
\item \textbf{double} \code{timebase}: La resolución temporal del TDC en segundos. El valor exacto es $8.09552722121028e^{-11}$s.
\item \textbf{int32} \code{timestamp\_count}: El tamaño del búfer de eventos. El búfer es el número total de eventos independientes que el TDC puede procesar en su memoria interna; si este valor está fijado en $1000$ y llega un evento $1001$, reescribirá al primero evento y así continuará. Entre $0$ y $1,000,000$.
\item \textbf{int32} \code{channels\_enabled}: La \textit{máscara de bytes} de los canales habilitados del TDC. Si sólo se habilita el canal 1 (correspondiente a una máscara 1), todos los eventos que lleguen a un canal que no sea el 1 no serán procesados. Se recomienda dejar todos los canales habilitados.
...
\end{table}
\item \textbf{int32} \code{timestamps}: Array de longitud \code{timestamp\_count} con las etiquetas de tiempo guardadas al momento.
\item \textbf{int32} \code{channel}: Array de longitud \code{timestamp\_count} con la máscara de bytes de los canales correspondientes a cada etiqueta de tiempo registrada.
\item \textbf{int32} \code{coincWin}: Coincidence window. La ventana de tiempo para el conteo de coincidencias En múltiplos de \code{timebase}.
\item \textbf{int32} \code{expTime}: Exposure time. El tiempo de integración para el cálculo de coincidencias. En milisegundos.
\item \textbf{int32} \code{bincount}: Parámetro del histograma de diferencias temporales. Número de barras del histograma. En múltiplos de \code{timebase}, entre $1$ y $1,000,000$.
\item \textbf{int32} \code{binsize}: Parámetro del histograma de diferencias temporales. Ancho de cada barra del histograma. En múltiplos de \code{timebase}, entre $2$ y $1,000,000$.
\end{itemize}

La lista completa de funciones implementadas para la paquetería de \code{hunahpy}, junto con su descripción, es:

\begin{enumerate}
\item \textbf{error code} \code{switch(byte)}: Función para debuggear el código. Esta fución se ejecuta después de casi todas las otras funciones y es la encargada de imprimir los códigos de error generados por cada función. En la Tabla \ref{switch} están todos los mensajes de error posibles y algunas notas sobre cómo corregirlos.
...
\item \textbf{error code} \code{close()}: Cierra la conexión al módulo TDC. Sólo se permite una conexión a la vez - sólo un programa de adquisición abierto. Es importante cerrar la conexión antes de desconectarlo para evitar dañar el módulo.
...
\item \textbf{error code} \code{configureSelfTest(channel,signal period,burst size,burst distance)}: El FPGA del TDC puede generar trenes de pulsos virtuales que serán procesados como señales añadidas a las entradas reales de cada canal.
\begin{itemize}
...
\end{figure}
\item \textbf{int8} \code{getChannel(byte)}: Regresa el número real de un canal basado en la máscara de bytes.
...
\item \textbf{error code} \code{getDataLost()}: Verifica si hay pérdidas de datos causadas por una velocidad de detección demasiado alta para el cable USB. De haber datos perdidos, el valor de \code{data\_loss} queda como $1$ y se levanta un mensaje de aviso al usuario.
...
\end{itemize}
\item \code{saveTimestamps(filename\_timestamps, filename\_channels,} \code{extension)}: Guarda los valores de \code{timestamps} y \code{channels} en un archivo con nombre \code{filename\_timestamps} y \code{filename\_channels}. Sólo puede guardar datos en formato decimal.
\item \code{writeTimestamps(filename, binary)}: Crea o abre un archivo con nombre \code{filename} y empieza a escribir todos los eventos registrados en él. Para formato binario, usar el argumento \code{binary=True}. Para dejar de escribir, llamar a la función de nuevo \textbf{sin} argumentos. Los archivos creados no tienen un límite de tamaño.
\textbf{IMPORTANTE:} Los archivos de datos guardados en formato binario \textit{no son legibles por un humano}. Es necesario decodificarlos primero. Cada evento consiste en 10 bytes: 8 bytes que determinan el tiempo de cada evento (en múltiplos de 81 ps) y 2 bytes que determinan su canal (de 0 a 7).
...
La función principal para guardar los datos es en este caso \code{getLastTimestamps}. En el archivo de configuración, se definió un tamaño de búfer de 100000 eventos y un total de archivos de 20. Escribirá los datos en formato decimal.
