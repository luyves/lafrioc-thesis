...
\end{itemize}
\item \code{saveTimestamps(filename_timestamps, filename_channels,} \code{filename-\\ extension)}: Guarda los valores de \code{timestamps} y \code{channels} en un archivo con nombre \code{filename_timestamps} y \code{filename_channels} respectivamente con extensión \code{filename_extension}.
\end{enumerate}
