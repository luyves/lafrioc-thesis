...
Para una fuente de luz térmica con ensanchamiento Doppler se puede hacer un análisis similar. Partiendo de la expresión del campo eléctrico para este tipo de luz:
\begin{equation*}
...
Vemos que la magnitud de $\g{1}(\tau)$ para luz térmica ensanchada por colisiones tiene un perfil Lorentziano, mientras que para luz térmica ensanchada por Doppler presenta un perfil Gaussiano. $|\g{1}(0)| = 1$ es el máximo y decrece monotónicamente hacia $0$.
...
De acuerdo con esta descripción clásica de la luz, es posible probar que:
\begin{align*}
1 &\leq \g{2}(0)\\
\g{2}(\tau) &\leq \g{2}(0).
\end{align*}
...
Para las fuentes de luz térmica se puede demostrar que, si se tiene un número $n$ muy grande de átomos, es válido:
\begin{equation}
...
Usando \ref{eq22} en la definición de $\g{2}$, tenemos que para luz térmica (y para $n>>1$):
\begin{equation}
\nonumber \g{2}(\tau) = 1+|\g{1}(\tau)|^2,
\end{equation}
quedando para luz térmica con colisiones y con ensanchamiento Doppler:
\begin{align}
