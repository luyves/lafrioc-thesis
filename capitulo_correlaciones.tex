...
La paquetería de uso para el módulo etiquetador de cuentas temporales (TDC) \textit{id800} está escrita en Python 3.6. Esta paquetería hace fuerte uso de la paquetería de \code{ctypes} que es una biblioteca de funciones foráneas para Python. Esta paquetería provee compatibilidad de estructuras con C/C++ y el uso de librerías compartidas (DLLs).

El módulo TDC venía con un software de control proprietario, un software de LabView y ejecutables compilados en C. El uso de \code{ctypes} fue necesario, pues el lenguaje estánar de programación del Laboratorio de Átomos Fríos es Python 3.6 y estandarizar el código usado por los programas de los experimentos es necesario.

Un pequeño manual se puede encontrar en el Anexo A y la paquetería está disponible en \textbf{REFERENCIA GITHUB} para uso público.
